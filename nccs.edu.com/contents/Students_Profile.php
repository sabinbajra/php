...
<?php
	...
	$query = "SELECT * FROM tbl_members, tbl_students WHERE tbl_members.id=tbl_students.id AND username <> '".$_SESSION["valid_user"]."' order by firstname ASC LIMIT " . $start . ", 50";
	$result = mysql_query($query);
		
	// Gives total number of members...
	$qu = "SELECT * FROM tbl_students";
	$re = mysql_query($qu);
	...
	$first = $start+1;
	 ...
	 if ($num == 0) {
		echo "<span class=\"text\">&nbsp;No students added</span>";
	}
	else {
		if ($num == 1) {
			$Total =  "<span class=\"text\">&nbsp;No. of student: $first - $finish of $count</span>";	
		}
		else {
			$Total =  "<span class=\"text\">&nbsp;No. of students: $first - $finish of $count</span>";
		}
	}
	?>
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
	  <tr>
		...
		<td width="50%" valign="top">
          ...
          <input name="imageField" type="image" src="../images/icons/search.gif" alt="Search Student(s) by FirstName" align="middle" width="16" height="16" border="0">
		  </div>
		  </form>
		</td>
	  </tr>
        ...
        <td colspan="2">
		  <table width="510" border="0" align="center" cellpadding="3" cellspacing="0">
			...
			<tr> 
			  <td colspan="6"> 
				<?
					$q = "SELECT count(*) as count FROM tbl_students";
					$r = mysql_query($q);
