<?
	...
	$id = $_GET['id'];
	$query = "Select * from tbl_news_articles where id = $id";
	$result = mysql_query($query);
	$show = mysql_fetch_array($result);
	
		echo "<br><form action=\"?AdminPageID=Contents&List=ListingDB&EditDB=DatabaseENF&NewsArticles=Edit&id=$id\" method=\"post\">\n";
		echo "<input type=\"hidden\" name=\"id\" value=\"$show[id]\">\n"
			...
			."  </tr>\n"
			."  <tr>\n"
			."	<td class=\"text\" valign=\"top\">News/Articles</td>\n"
			...
			."	<td class=\"text\"><input type=\"text\" name=\"submitted_by\" size=\"50\" value=\"$show[submitted_by]\" class=\"search_box1\"></td>\n"
			."  </tr>\n"
			."  <tr>\n"
			."	<td class=\"text\">&nbsp;</td>\n"
			."	<td><input type=\"submit\" name=\"submit\" value=\"Edit\" class=\"search_box1\">";
			if ($EditDB == "DatabaseENF") {
				echo "<input type=\"button\" name=\"Submit\" value=\"Ok\" onClick=\"window.location='?AdminPageID=Contents&List=ListingDB&ViewDB=NewsArticles'\" class=\"search_box1\">";
			}
		echo "</td>\n"
			."  </tr>\n"
			."</table>"
