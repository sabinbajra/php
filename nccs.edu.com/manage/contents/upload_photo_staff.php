...
<?
	...
	$query = "SELECT firstname from staff where id=$id";
...
<form action="upload_enf_staff.php?id=<?=$id;?>" method="post" enctype="multipart/form-data" name="upload" onSubmit="return form_Validator(this)">
  <?
		if($le == 1)
			{
				if($er == 1)
