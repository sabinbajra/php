<?
	...
	$query = "SELECT firstname from tbl_members where id=".$_SESSION['valid_id'];
	$result = mysql_query($query);
	$show = @mysql_fetch_array($result);
?>
<title>Upload Photo! <?=$show[firstname];?>... </title>
<script language="JavaScript1.2" src="../include/validator.js"></script>
<link href="../include/bcis.css" rel="stylesheet" type="text/css">
<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0">
<form action="upLoad_ENF.php?MyPhoto=upLoad&id=<?=$_SESSION['valid_id'];?>" method="post" enctype="multipart/form-data" name="upload" onSubmit="return form_Validator(this)">
  <?
		if($le == 1)
			{
				if($er == 1)
